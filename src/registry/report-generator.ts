...
function generateReport(
	...
	lines.push(`### Scanner`);
	lines.push(``);
	lines.push(`[AgentVerus Scanner](https://github.com/agentverus/agentverus-scanner) v${summary.scannerVersion} performs static analysis across five categories:`);
	lines.push(``);
	lines.push(`1. **Permissions** (25%) — Does the skill declare what access it needs? Are the declarations justified?`);
	lines.push(`2. **Injection** (30%) — Does the skill contain prompt injection, jailbreak attempts, or instruction manipulation?`);
	lines.push(`3. **Dependencies** (20%) — Does the skill reference suspicious URLs, domains, or external services?`);
	...
	lines.push(`- Patterns in **code blocks** (examples) receive 30% severity`);
